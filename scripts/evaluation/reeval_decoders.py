...
def load_segmenter(
    ...
    if model_type == "crf":
        ...
        if arch == "xlstm":
            ...
            # encoder.fwd.*.rate + encoder.fwd.*.inner.*; multi-rate (Exp D) -> encoder.levels.<i>.fwd.<j>.*.
            if any(k.startswith("model.encoder.levels.") for k in state):
                stem = "model.encoder.levels.0.fwd."  # level 0's forward stack
                extra = {
                    "hidden_size": state[f"{stem}0.q.weight"].shape[0],
                    "num_heads": state[f"{stem}0.fgate.weight"].shape[0],
                    "num_layers": len({k.split(".")[5] for k in state if k.startswith(stem)}),
                    "multirate": True,  # rates default to (1,4,16) — the trained config
                }
                input_size = state[f"{stem}0.q.weight"].shape[1]
            else:
