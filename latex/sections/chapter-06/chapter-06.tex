...
neuronales. Combinadas logran formar las \gls{dnn} (\textit{deep learning neural network}).
...
por la función \gls{relu} (\textit{Rectified Linear Unit}).

\begin{figure}[H]
  \centering
  \includegraphics[scale=0.3]{sections/chapter-06/images/ReLU.png}
  \caption[Función de activación \gls{relu}.]{Función de activación \gls{relu}. El línea sólida se muestra
...
probabilidad de que una entrada sea igual a cero es casi nula. La función \gls{relu} también tiene la propiedad
...
encuentre activa. Muchas variaciones de la función \gls{relu} consideran mantener a las unidades siempre
...
\indent \gls{mlp} (Multilayer perceptron) tienen una arquitectura básica. Cada unidad o neurona de una capa se
...
\begin{itemize}
  \item Error cuadrático medio (\gls{mse})\footnote{La función de costo \gls{mse} se conoce también como:
  \item \textit{Quadratic Loss} o \textit{L2 loss}}
  \begin{align}
    \mathbf{MSE} = \frac{\sum_{i=1}^n (y_i - \hat{y}_i)^2}{n}
  \end{align}

  \item Error absoluto medio (\gls{mae})\footnote{Se conoce a la función de costo \gls{mae} como:
  \item \textit{L1 loss}}
  \begin{align}
    \mathbf{MAE} = \frac{\sum_{i=1}^n |y_i - \hat{y}_i|}{n}
  \end{align}

  \item Error de sesgo medio (\gls{mbe})
  \begin{align}
...
herramienta básica en todos estos algoritmos es lo que se conoce como \gls{sgd} (\textit{Stochastic Gradient
...
iterando. \\
\indent El algoritmo \gls{sgd} consiste en computar el gradiente. Para ello se considera la técnica de
...
% \indent Existen problemas, donde los \gls{mlp} no resuelven determinadas cuestiones y no se adaptan. Los perceptrones multicapa tienene como entrada vectores, los cuales en los casos que la información proviene de imágenes, deberían transformarse las imágenes en vectores perdiendo la forma de noción espacial que proveen éstas. Las redes convolucionales actuan directamente sobre matrices o tensores con tres canales \gls{rgb}. Actualmente las \gls{cnn}, se utilizan para clasificación, segmentación, reconocimiento de objetos y reconocimiento facial.

% \subsection{Capas en una red convolucional}

% \indent Una \gls{cnn} puede estar compuesta por distintas capas intermedias: capas convolucionales, \textit{pooling layers}, \textit{fully conected layers}.
...
pasando información a su sucesora. La primer red neuronal recurrente, era una \gls{mlp} con un bucle hacia si
...
otros. Este éxito se debe a la eficiencia de las redes \gls{lstm}, que es una especie de red recurrente. Las
redes \gls{lstm} fueron introducidas por Hochreiter y Schmidhuber (1997) \cite{pp:hochreiter-schmidhuber} y
fueron creadas con el propósito de aprender dependencias a largo plazo. Una celda \gls{lstm} comprende, en un
...
\gls{lstm}. Una celda RNN contiene sólo una capa. En cambio, la celda \gls{lstm} contiene 4 capas dadas
...
\begin{figure}[H]
  ...
  \caption[Diagrama de una red \gls{lstm}]{Diagrama de una red \gls{lstm}. Imagen extraída de
  \href{http://colah.github.io/posts/2015-08-Understanding-\gls{lstm}s/}{\textit{"Understanding \gls{lstm}
  Networks"}}}
  \label{fig:lstm-schematic}
\end{figure}

\indent Las redes \gls{lstm} son las elegidas en este trabajo. La particularidad de estas redes, como se ha
...
\begin{itemize}
  \item Sensibilidad
  \begin{align}
    \gls{tpv} = \frac{\gls{tp}}{\gls{tp}+\gls{fn}}
  \end{align}

  \item Especificidad
  \begin{align}
    \gls{tnr} = \frac{\gls{tn}}{\gls{tn}+\gls{fp}}
  \end{align}

  \item Precisión
  \begin{align}
    \gls{ppv} = \frac{\gls{tp}}{\gls{tp}+\gls{fp}}
  \end{align}

  \item Valor Predictivo Negativo
  \begin{align}
    \gls{npv} = \frac{\gls{tn}}{\gls{tn}+\gls{fn}}
  \end{align}

  \item Tasa de error
  \begin{align}
    \gls{fnr} = \frac{\gls{fn}}{\gls{fn}+\gls{tp}}
  \end{align}

  \item Tasa de falsos positivos
  \begin{align}
    \gls{fpr} = \frac{\gls{fp}}{\gls{fp}+\gls{tn}}
  \end{align}

  \item Tasa de descubrimiento
  \begin{align}
    \gls{fdr} = \frac{\gls{fp}}{\gls{fp}+\gls{tp}}
  \end{align}

  \item Tasa de omisión
  \begin{align}
    \gls{for} = \frac{\gls{fn}}{\gls{fn}+\gls{tn}}
  \end{align}

  \item Exctitud
  \begin{align}
    \gls{acc} = \frac{\gls{tp}+\gls{tn}}{\gls{tp}+\gls{tn}+\gls{fn}+\gls{fp}}
  \end{align}

  \item Puntaje F$_1$
  \begin{align}
    F_1 = \frac{2\gls{tp}}{2\gls{tp}+\gls{fp}+\gls{fn}}
  \end{align}
\end{itemize}
...
publicados. \\
\indent \gls{tp} define a los verdaderos positivos y \gls{tn} a los verdaderos negativos. En base a estos
dos parámetros se pueden calcular todas las métricas mencionadas. Por otro lado, cabe destacar que dependiendo de la
cantidad de clases que el problema posea, es necesario computar \gls{tp} y \gls{tn} para cada una de ellas
